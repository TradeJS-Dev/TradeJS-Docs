...
Локальный фильтр пакета допускает только сигналы `SHORT`. Зафиксированное
правило H2 отклоняет слабое сочетание `near_support` и `btc_lead`, а также
требует `swingAmplitudeAtr >= 4.5` и `breakoutDistanceAtr >= 0.22`. Остальные
проверки относительной силы и геометрии канала тоже должны пройти.
...
| Риск | `RISK_FEE_RATE`, `RISK_SLIPPAGE_BPS`, `RISK_MARKET_IMPACT_BPS`, `MAX_LOSS_VALUE`, `FLAG_TARGET_POLE_RATIO`, `FLAG_STOP_BUFFER_ATR`, `FLAG_EXIT_ON_OPPOSITE_PATTERN` | Задают оценки комиссии, проскальзывания и влияния на рынок, размер позиции, проекцию цели, запас стопа и выход по противоположной модели. |
