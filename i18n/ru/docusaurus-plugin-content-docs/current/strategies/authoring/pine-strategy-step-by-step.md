...
Если нужен путь без Pine, см. [TypeScript-стратегия через `StrategyAPI`](./ma-strategy-step-by-step).
...
Контракт plot-серий, который читает `core.ts`:
...
## 8. Зарегистрируйте стратегию в built-in реестре

Обновите [`packages/core/src/strategy/manifests.ts`]:
...
После этого `AdaptiveMomentumRibbon` доступна runtime/backtest как обычная стратегия.
