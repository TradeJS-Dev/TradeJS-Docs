...
`tradejs.config.ts`. Зафиксируйте точный пакет стратегии и lock-файл, затем
проверьте счёт, коннектор, набор инструментов и ограничения риска. Не добавляйте
ручную версию: проверки Project разбирают полную конфигурацию и вычисляют оба
runtime identifier.
...
Сохраните полученные `strategyRevision` и `deploymentCompositionId` вместе с
evidence кандидата.
