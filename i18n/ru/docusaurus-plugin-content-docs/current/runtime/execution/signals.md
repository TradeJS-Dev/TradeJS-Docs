...
Для криптовалют базовое имя `BTC` в `selection.tickers` также соответствует
нормализованному имени `BTCUSDT` в среде исполнения.
...
## Фиксация двух версий одной стратегии

Если двум развёртываниям нужны разные версии одной стратегии, установите каждую
точную версию под отдельным npm-псевдонимом:
...
В каждой декларации укажите нужный псевдоним в `module`. Ключ стратегии
остаётся `TrendFollow`, а каждое развёртывание использует свой счёт, версию
пакета и полную конфигурацию. TradeJS проверяет цель псевдонима и точную
установленную версию. Обычное обновление стабильных пакетов не меняет версии,
установленные через псевдонимы.
