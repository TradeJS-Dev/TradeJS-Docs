...
Веб-приложение показывает записанную в Git конфигурацию отдельно от текущего
состояния исполнения. В панели конфигурации также видны состояние процесса,
разрешение на размещение ордеров, соединение и пауза. График берёт смены
ревизий из событий активации развёртывания, а не из первой сделки после
изменения. Красная отметка означает смену пакета стратегии или его версии.
Оранжевая отметка означает другое изменение вычисленной ревизии. Действие
**Revisions** показывает эти активации, даже если в момент активации стратегия
не совершала сделок. Доходность, доля успешных сделок, просадка, время в позиции и
другие показатели на карточке стратегии считаются только по закрытым сделкам.
Открытые сделки остаются в сводке и списке ордеров, но их нереализованный PnL
не влияет на эти показатели до закрытия. Исследовательские отчёты могут
создаваться локально или в CI, но рабочему серверу они не нужны для запуска.
