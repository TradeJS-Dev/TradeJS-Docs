...
description: 'Настройте дополнительный AI-слой TradeJS, который оценивает сигналы стратегии, сохраняет происхождение промптов и может ограничивать исполнение ордеров.'
---
...
Runtime использует fallback `4`, если стратегия не задала threshold. Built-in
стратегии задают `MIN_AI_QUALITY` в собственных config, поэтому проверяйте
effective strategy config и не рассчитывайте на единый глобальный default.

Примечания:

- `AI_API_KEY` и `AI_API_ENDPOINT` должны задаваться в записи пользователя, а не в app environment variables.
