...
Если нужно проверить изменения промптов до выхода в live, см. [Replay промптов из бэктестов](./prompt-replay). Этот workflow использует AI-строки, выгруженные из бэктеста, чтобы прогнать текущую prompt-логику на том же историческом срезе.
