...
- `runtime-evidence` — сбор журнала текущего контракта deployment v2 / lineage v3;
- `runtime-evidence-sync` — проверка текущего контракта и импорт журнала с другого сервера;
...
- `runtime-feedback-notify` — отправка в Telegram проверенной сводки из неизменяемого набора;
...
## `runtime-feedback-notify`

`runtime-feedback-notify` проверяет неизменяемый набор с результатом
воспроизведения и отправляет сводку сверки в Telegram пользователя, указанного
в наборе.

- `--bundle` — каталог проверенного набора, обязательно;
- `--dryRun` — проверить набор и напечатать сообщение без отправки.

Команда проверяет манифест, файлы и контрольные суммы. Набор без данных
сравнения отклоняется и не превращается в ошибочный чистый отчёт.
