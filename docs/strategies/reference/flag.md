...
| Risk | `RISK_FEE_RATE`, `RISK_SLIPPAGE_BPS`, `RISK_MARKET_IMPACT_BPS`, `MAX_LOSS_VALUE`, `FLAG_TARGET_POLE_RATIO`, `FLAG_STOP_BUFFER_ATR`, `FLAG_EXIT_ON_OPPOSITE_PATTERN` | Estimate one-way fees, slippage, and market impact, size positions, project the target, buffer the stop, and choose the opposite-pattern exit. |
